...
export default function PromptPage() {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ Extract name and description from received JSON
      const { name, description } = data

      if (!name || !description) {
        throw new Error('Missing name or description in response JSON')
      }
      ...
      // ✅ Send to second API endpoint
      const saveResponse = await fetch('https://dev295033.service-now.com/api/1781717/create_bot/create-bot', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!saveResponse.ok) throw new Error('Failed to save flow data')

      toast.success('Flow data saved successfully!')
